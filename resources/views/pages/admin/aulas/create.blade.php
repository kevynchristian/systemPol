...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-white glitch" data-text="CRIAR FORMAÇÃO">CRIAR FORMAÇÃO</h1>
    ...
    <div class="card-tactical shadow mb-4">
        <div class="card-header-tactical py-3">
            <h6 class="m-0 font-weight-bold"><i class="fas fa-plus-circle"></i> DETALHES DA FORMAÇÃO</h6>
        </div>
        <div class="card-body-tactical">
            <form action="{{ route('admin.aulas.store') }}" method="POST">
                ...
                <div class="form-group row mb-4">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control input-tactical" id="name" name="name" value="{{ old('name') }}" required>
                    </div>
                </div>
                    ...
                    <div class="col-sm-10">
                        <select class="form-control input-tactical select2-funcoes" id="required_permission" name="required_permission" style="width: 100%;">
                            ...
                            @foreach($funcoes as $funcao)
                                <option value="{{ $funcao->name }}" {{ old('required_permission') == $funcao->name ? 'selected' : '' }}>
                                    {{ $funcao->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-sm-10">
                        <select class="form-control input-tactical select2-prerequisite" id="prerequisite_id" name="prerequisite_id" style="width: 100%;">
                            ...
                            @foreach($aulas as $aulaItem)
                                <option value="{{ $aulaItem->id }}">{{ $aulaItem->name }}</option>
                            @endforeach
                        </select>
